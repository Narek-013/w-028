...
const COLLECTION = 'clock_translations';
const LANGUAGES_COLLECTION = 'languages';
const GROUP_COLLECTION = 'settings';

const LABEL_FIELDS = ['label_days', 'label_hours', 'label_minutes', 'label_seconds'];
    ...
    await client.request(
      createCollection({
        ...
        meta: {
          ...
          note: 'Countdown clock labels per language',
          versioning: false,
        },
        schema: {
          schema: 'public',
...
async function ensureLanguageRelation(client) {
  await ensureField(client, {
    ...
    meta: {
      ...
      sort: 1,
      note: 'Language for these countdown labels',
    },
  });
...
async function upsertTranslation(client, code, translations) {
  ...
  if (!languageId) {
    console.warn(`Language "${code}" not found; skipping clock translation seed.`);
    return 'skipped';
  }
...
export async function up(client) {
  ...
  const clockRead = await ensurePublicReadPermission(client, COLLECTION);

  console.log(
    `Clock translations ready (${COLLECTION}): collection=${collectionStatus}, seed=[${results.join(', ')}], public_read=${clockRead}`,
  );
}
